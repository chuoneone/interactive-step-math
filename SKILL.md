...
## 7. 檢查答案與計分規則 (100 分制與答錯嚴格扣分)

- **💯 滿分 100 分總分制**：
  - 全卷練習總分為 100 分。
  - 系統於初始化時統計整份測驗全部題目的總步驟數 (`totalStepsInQuiz`)。
  - 每個無失誤答對的步驟按比例取得分數：
    $$\text{總得分} = \text{Math.round}\left(\frac{\text{全卷答對無失誤步驟總數}}{\text{全卷總步驟數}} \times 100\right)$$
  - 頂部導覽列即時更新：`<span id="scoreDisplay">0</span> / 100 分`。
- **🚫 嚴格防猜與答錯懲罰 (Strict Error Penalty)**：
  - 學生只要在任一步驟按錯選項（`stepMistakes[stepIndex] = true`），該步驟在後續完成時**記為 0 分（不得加分）**。
  - **答錯不清除已填寫內容**：提示「再想想看喔！」，保留已填內容讓學生點擊 slot 訂正。
  - 學生必須訂正到完全正確，才能解鎖並進入下一步。
- **➕ 步驟一符號判斷直接合併**：
  - 涉及正負號判斷或去括號時，**第一步直接合併正負號填答**（例如：`= [ ? ] (25 × 4) × 17` 提供 `+`、`-` 選項），避免冗長無趣的「數負號個數」純文字步驟。
- **答對 (Correct)**：
  - 綠色提示 + Tada / Bounce 動畫與綠色完成標記。
  - 播放 Web Audio API 清脆答對提示音。
  - 解鎖下一步驟並平滑捲動。
