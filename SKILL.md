...
description: 💻互動式數學 (選項點擊填入 / 鍵盤輸入解題工具) - 擁有者/來源：朱一 (pppchin7@gmail.com)。當使用者提供數學題目或圖片，並要求「互動式數學」、「點擊版數學」、「輸入版數學」、「選項點擊填入」、「做成互動數學網頁」或要求製作互動功能時觸發。Agent 會在未指定模式或題數時，主動使用 ask_question 詢問使用者要「點擊填入版」還是「鍵盤輸入版」以及「需要生成幾題（單題/多題練習）」，並依選擇產出具備逐步解鎖 (Step-by-step unlock)、KaTeX 漂亮渲染、Tailwind 響應式佈局、答錯訂正防呆、Web Audio 提示音效與完成結算視窗的單一 HTML 互動教學網頁。
---

# 💻 互動式數學 (Interactive Math: Click / Input)

> **擁有者 / 來源**：朱一 (`pppchin7@gmail.com`)  
> **簡介與說明**：貼上題目或圖片後，送出。支援「選項點擊填入式」與「鍵盤輸入式」兩種互動解題工具的 HTML 教學網頁。
